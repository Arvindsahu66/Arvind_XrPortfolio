...
import { About, Acheivements, Contact, Experience, Hero, Navbar, Tech, Works } from "@/components";
import dynamic from "next/dynamic";

// Load StarsCanvas client-side only (depends on three/@react-three/drei)
// Load StarsCanvas directly (file-level) so the canvas barrel isn't imported server-side
const StarsCanvas = dynamic(() => import("@/components/canvas/Stars").then((m) => m.default), { ssr: false });
...
function App({ loading }) {
  ...
  return (
    <main className="relative z-0 w-full h-full">
      ...
      <section
  id="achievements"
  className="scroll-mt-24 relative z-0 w-full"
>
  <Acheivements />
</section>
      <Tech />
